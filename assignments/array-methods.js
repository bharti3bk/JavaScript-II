...
let allCaps = [];
 ...
 var upperFirstName = runners.map((runner) => {
 var myvariable = runner.first_name.toUpperCase();
 allCaps.push(myvariable);  
}); 
...
let largeShirts = [];
var sizeLarge = runners.filter((runners)=> {
    if(runners.shirt_size === "L")
    {
        var temp = runners.shirt_size;
        
    }  

    largeShirts.push(temp);
}); 
...
var donationAdd = runners.reduce((accumulator , currentValue) => 
  
   sumDonation = accumulator + currentValue.donation , 0
  
);  
ticketPriceTotal.push(sumDonation);
